...
export class App {
  ...
  async obtainCertFor(hostname: string, deployId: string) {
    ...
    try {
      ...
      const certbotInfo = await util.promisify(exec)(`/usr/bin/certbot run -d ${hostname} --webroot --preferred-challenges http --agree-tos --webroot-path ${webRootPath} --fullchain-path ${certPath} --key-path ${keyPath}`);
